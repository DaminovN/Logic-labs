...
class deducer alpha assumptList ic oc = 
	let assumption_hash_table = Hashtbl.create 255 in
	let right_tree_list_map = Hashtbl.create 255 in
	let tree_ind_map = Hashtbl.create 255 in
	let index_table = Hashtbl.create 255 in
	let j = ref 0 in
	let k = ref 0 in

	let part1 = Binop(Impl, alpha, Binop(Impl, alpha, alpha)) in
	let part2 = get_ax2 alpha (Binop(Impl, alpha, alpha)) alpha in
	let part3 = Binop(Impl, Binop(Impl, alpha, Binop(Impl, Binop(Impl, alpha, alpha), alpha)), Binop(Impl, alpha, alpha)) in
	let part4 = Binop(Impl, alpha, Binop(Impl, Binop(Impl, alpha, alpha), alpha)) in
	let part5 = Binop(Impl, alpha, alpha) in
	...
	method start () = 
		try
			...
			let rec solve cur_id =
				let my_expr = self#get_tree (List.nth assumptList cur_id) in
